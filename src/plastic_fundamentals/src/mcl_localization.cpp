...
void updateParticlesMotion(double delta_x, double delta_y, double delta_theta) {
    // Create distributions for motion noise
    std::normal_distribution<double> x_noise(0, MOTION_X_NOISE);
    std::normal_distribution<double> y_noise(0, MOTION_Y_NOISE);
    std::normal_distribution<double> theta_noise(0, MOTION_THETA_NOISE);
    
    for (auto& p : particles) {
        double noise_x = x_noise(random_generator) * std::abs(delta_x);
        double noise_y = y_noise(random_generator) * std::abs(delta_y);
        double noise_theta = theta_noise(random_generator) * std::abs(delta_theta);
...
void moveLinear(ros::ServiceClient& diffDrive, double distance_m, double wheel_speed_rad_s) {
    ...
    double delta_x = distance_m * cos(0); // Assuming robot is aligned with x-axis
    double delta_y = distance_m * sin(0);
    updateParticlesMotion(delta_x, delta_y, 0);
}
