...
void scanCallback(const sensor_msgs::LaserScan::ConstPtr& msg) {
    ...
    if (perpendicularWalls) {
        ...
        if (rotate_tries < 1) {
            rotate(M_PI, 5.0); // Turn 180 degrees if no lines found
            ros::Duration(0.5).sleep();
            rotate_tries++;
        }
        else if(rotate_tries < 2){
            rotate(M_PI / 2, 5.0); // Turn 90 degrees if no lines found
            ros::Duration(0.5).sleep();
            rotate_tries++;
        }
        else if (parallelWalls) {
            // Assume two lines are present
            const Line& l1 = parallel_lines[0];
            const Line& l2 = parallel_lines[1];

            // Normal vector from l1 (same direction as walls)
            float a = l1.a_dash;
            float b = l1.b_dash;
            float norm = sqrt(a * a + b * b);
            float na = a / norm;
            ...
            // Direction along the corridor (perpendicular to normal)
            float dir_x = -nb;
            float dir_y = na;

            // Rotate to align with corridor
            float desired_angle = atan2(dir_y, dir_x);
            while (desired_angle > 3.14159265358979323846) desired_angle -= 2 * M_PI;
            while (desired_angle < -3.14159265358979323846) desired_angle += 2 * M_PI;

            ROS_INFO("Aligning parallel to corridor walls. Heading: %.2f rad", desired_angle);
            rotate(desired_angle, 5.0);

            // Move forward into next cell
            float forward_distance = 0.8;  // center to center
            translate(forward_distance, 5.0);

            // Mark that we will try aligning again at new pose
            ROS_INFO("Moved to next cell, retrying alignment...");
            ros::Duration(0.5).sleep(); // Let robot settle

            // Re-attempt alignment by re-triggering scan or function
            // You can use a flag or directly call a function
            centered = false;
            processing_done = false;
            rotate_tries++;

            return;  // Exit to wait for next scan callback
        }
        else {
            rotate(M_PI / 2, 5.0);
            ros::Duration(0.5).sleep();
